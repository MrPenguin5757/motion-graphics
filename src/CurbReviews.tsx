...
const BASE = [108, 96, 108, 102, 108, 96, 120];
...
const Cover: React.FC = () => {
  ...
  return (
    <RScene dark>
      <Pop delay={0}>
        <div style={{fontFamily: BRIC, fontWeight: 800, fontSize: 26 * u, lineHeight: 0.8, letterSpacing: '-0.04em'}}>5</div>
      </Pop>
      <Head dark size={9.4} delay={8} lines={[{t: 'Ways to get'}, {t: 'more Google'}, {t: 'reviews', accent: true}]} />
      <Pop delay={20} style={{width: '100%'}}>
        <div style={{display: 'flex', alignItems: 'center', gap: 2.4 * u, width: '100%', border: `${0.35 * u}px solid rgba(244,239,228,.18)`, borderRadius: 3 * u, padding: `${3 * u}px ${3.4 * u}px`}}>
          <svg width={6 * u} height={6 * u} viewBox="0 0 24 24" fill="none" style={{flex: '0 0 auto'}}>
            <path d="M12 2l2.9 6.1 6.6.8-4.9 4.6 1.3 6.6L12 17.8 6.1 20.7l1.3-6.6L2.5 9.5l6.6-.8z" fill={C.marigold} />
          </svg>
          <span style={{fontFamily: HANK, fontWeight: 700, fontSize: 3.2 * u, letterSpacing: '0.04em', textTransform: 'uppercase', lineHeight: 1.25}}>Reviews are why they call you first.</span>
        </div>
      </Pop>
    </RScene>
  );
...
const Tip1: React.FC = () => (
  <RScene dark={false}>
    <Num n={1} dark={false} />
    <Head dark={false} size={10.8} lines={[{t: 'Ask in person,'}, {t: 'not by text'}]} />
    <Rule dark={false} />
    <Body dark={false}>A face-to-face ask beats a text they'll swipe past. Ask right there in the driveway.</Body>
  </RScene>
);
...
const Tip2: React.FC = () => {
  ...
  return (
    <RScene dark>
      <Num n={2} dark />
      <Head dark size={9.4} lines={[{t: 'Ask when'}, {t: "they're"}, {t: 'happiest'}]} />
      <Rule dark />
      <Pop delay={19} style={{width: '100%'}}>
        <div style={{background: C.asphalt2, borderRadius: 3 * u, padding: `${3 * u}px ${3.4 * u}px`, width: '100%'}}>
          <b style={{display: 'block', fontFamily: BRIC, fontWeight: 800, fontSize: 8.8 * u, letterSpacing: '-0.03em', color: C.marigold, lineHeight: 1, marginBottom: 1 * u}}>5 min</b>
          <span style={{fontFamily: HANK, fontWeight: 500, fontSize: 3.2 * u, color: C.gravelDk, lineHeight: 1.35}}>right after you finish, while the work still looks brand new</span>
        </div>
      </Pop>
      <Body dark delay={30}>Wait till tomorrow and that feeling's gone.</Body>
    </RScene>
  );
};
...
const CRow: React.FC<{delay: number; yes: boolean; divide?: boolean; children: React.ReactNode}> = ({delay, yes, divide, children}) => {
  ...
  return (
    <Pop delay={delay} style={{width: '100%'}}>
      <div style={{display: 'flex', alignItems: 'center', gap: 2.4 * u, fontFamily: HANK, fontWeight: 700, fontSize: 4.4 * u, borderTop: divide ? '1px solid rgba(33,30,26,.10)' : 'none', paddingTop: divide ? 2.4 * u : 0}}>
...
const Tip3: React.FC = () => {
  ...
  return (
    <RScene dark={false}>
      <Num n={3} dark={false} />
      <Head dark={false} size={10.8} lines={[{t: 'Make it one'}, {t: 'tap'}]} />
      <Rule dark={false} />
      <div style={{display: 'flex', flexDirection: 'column', gap: 2.4 * u, width: '100%'}}>
        <CRow delay={17} yes>Text your direct review link</CRow>
        <CRow delay={23} yes divide>Keep a QR code in the truck</CRow>
        <CRow delay={29} yes={false} divide>Telling them to &ldquo;look us up&rdquo;</CRow>
      </div>
    </RScene>
  );
};
...
const Tip4: React.FC = () => {
  ...
  return (
    <RScene dark>
      <Num n={4} dark />
      <Head dark size={9.4} lines={[{t: 'Tell them'}, {t: 'what to'}, {t: 'mention'}]} />
      <Rule dark />
      <Body dark delay={17}>A vague ask gets a vague review.</Body>
      <In delay={24} style={{marginTop: 1.4 * u}}>
        <span style={{fontFamily: HANK, fontWeight: 800, fontSize: 3.2 * u, letterSpacing: '0.06em', textTransform: 'uppercase', color: C.curbDk}}>Say this instead:</span>
      </In>
      <In delay={30}>
        <p style={{margin: 0, fontFamily: BRIC, fontWeight: 800, fontSize: 6 * u, lineHeight: 1.05, letterSpacing: '-0.02em', color: C.sand, maxWidth: '98%'}}>&ldquo;Mention the driveway and it helps me a ton.&rdquo;</p>
      </In>
    </RScene>
  );
};
...
const Tip5: React.FC = () => (
  <RScene dark={false}>
    <Num n={5} dark={false} />
    <Head dark={false} size={10.8} lines={[{t: 'Ask every'}, {t: 'single time'}]} />
    <Rule dark={false} />
    <Body dark={false}>Ask once in a while, get six reviews. Make it the last step of every job, every time.</Body>
  </RScene>
);
...
const Outro: React.FC = () => {
  ...
  return (
    <RScene dark>
      ...
      <Rule dark delay={4} />
      <Body dark delay={14}>Quotes, jobs, routes, invoices. All in one app. Link in bio.</Body>
...
/* ---------- composition ---------- */
export const CurbReviews: React.FC = () => {
  const {width, height} = useVideoConfig();
